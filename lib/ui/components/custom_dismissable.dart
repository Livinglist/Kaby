...
class CustomDismissible extends StatefulWidget {
  ...
  final VoidCallback afterRebuild;
...
class CustomDismissibleState extends State<CustomDismissible> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  @override
  ...
  void initState() {
    super.initState();
    moveController = AnimationController(duration: widget.movementDuration, vsync: this)..addStatusListener(_handleDismissStatusChanged);
...
//  @override
//  bool get wantKeepAlive => moveController?.isAnimating == true || _resizeController?.isAnimating == true;
  ...
  DismissDirection get dismissDirection => _extentToDirection(_dragExtent);

  bool get _isActive {
    return _dragUnderway || moveController.isAnimating;
  }
  ...
  void _handleDragStart(DragStartDetails details) {
    ...
    if (moveController.isAnimating) {
      _dragExtent = moveController.value * _overallDragAxisExtent * _dragExtent.sign;
      moveController.stop();
    } else {
      _dragExtent = 0.0;
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    if (!_isActive || moveController.isAnimating) return;
  ...
  _FlingGestureKind _describeFlingGesture(Velocity velocity) {
    ...
    assert(dismissDirection != null);
    if (flingDirection == dismissDirection) return _FlingGestureKind.forward;
  ...
  Future<void> _handleDragEnd(DragEndDetails details) async {
    if (!_isActive || moveController.isAnimating) return;
    _dragUnderway = false;
    if (moveController.isCompleted && await _confirmStartResizeAnimation() == true) {
    ...
    switch (_describeFlingGesture(details.velocity)) {
      case _FlingGestureKind.forward:
        ...
        assert(!moveController.isDismissed);
        if ((widget.dismissThresholds[dismissDirection] ?? _kDismissThreshold) >= 1.0) {
          moveController.reverse();
          break;
        }
        _dragExtent = flingVelocity.sign;
        moveController.fling(velocity: flingVelocity.abs() * _kFlingVelocityScale);
        break;
      ...
      case _FlingGestureKind.none:
        if (!moveController.isDismissed) {
          ...
          if (moveController.value > (widget.dismissThresholds[dismissDirection] ?? _kDismissThreshold)) {
  ...
  Future<void> _handleDismissStatusChanged(AnimationStatus status) async {
    if (status == AnimationStatus.completed && !_dragUnderway) {
      if (await _confirmStartResizeAnimation() == true) {
        _startResizeAnimation();
        widget.onDismissed(dismissDirection);
      } else {
  ...
  Future<bool> _confirmStartResizeAnimation() async {
    if (widget.confirmDismiss != null) {
      final DismissDirection direction = dismissDirection;
  ...
  void _startResizeAnimation() {
    ...
    assert(moveController.isCompleted);
